...
with tabs[0]:
    ...
    if events_df.empty:
        ...
        rag_summary_files = sorted(
        evaluation_results_dir.glob(
            "rag_summary_*.json"
        ),
        reverse=True,
    )

    governance_summary_files = sorted(
        evaluation_results_dir.glob(
    ...
    if rag_summary_files and governance_summary_files:

        latest_rag_summary_path = (
            rag_summary_files[0]
        )

        latest_governance_summary_path = (
            governance_summary_files[0]
        )
        ...
        with open(
            ...
            governance_summary = json.load(file)
        ...
        st.info(
            ...
            "Run evaluation/ragas_eval.py first."
        )
...
with tabs[6]:
    ...
    if events_df.empty:
        ...
        audit_df = events_df[available_audit_columns].copy()
